...
if (!class_exists(Clvr_Vyfakturuj)){
    ...
    public function store_payment_meta($payment_meta){
      ...
      return $payment_meta;
		...
		public function getVyfakturujCustomer($payment_id){
    		...
    		'company' => $payment_meta['edd_firma'],
    		'street' => $payment_data['edd_ulice'],
    		'city' => $payment_data['edd_mesto'],
    		'zip' => $payment_data['edd_psc'],
    ...
    */
    ...
    public function getInvoice( $payment_id, $proforma=false ) {
      ...
      add_post_meta( $payment_id, self::INVOICE_ID_META_KEY, $inv['id'] );
      return $inv;
    }
    ...
    private function prepareDataForInvoice($payment_meta, $payment_id, $proforma = false){
        ...
        $opt = array(
			...
			'customer_name' => $this->getCustomerName($payment_meta['edd_firma'],$payment_meta['user_info']['first_name'],$payment_meta['user_info']['last_name']),
			'webhook_paid' => get_home_url() .'?edd-listener=eddvyfakturuj&paymentid=' . $payment_id,
			'VS' => $payment->number
			  
        );
      	if(isset($payment_meta['edd_ic'])){
      		$opt['customer_IC'] = $payment_meta['edd_ic'];
      	}
      	if(isset($payment_meta['edd_dic'])){
      		$opt['customer_DIC'] = $payment_meta['edd_dic'];
      	}
      	if(isset($payment_meta['edd_ulice'])){
      		$opt['customer_street'] = $payment_meta['edd_ulice'];
      	}
      	if(isset($payment_meta['edd_mesto'])){
      		$opt['customer_city'] = $payment_meta['edd_mesto'];
      	}
      	if(isset($payment_meta['edd_psc'])){
      		$opt['customer_zip'] = $payment_meta['edd_psc'];
      	}
      	if(isset($payment_meta['edd_stat'])){
      			$opt['customer_country'] = $payment_meta['edd_stat'];
		}
      ...
      private function prepareInvoiceItems($cart){
          ...
          foreach($cart as $cart_item){
            ...
            $vyfakturuj_item = array(
			  ...
			  'vat' => $cart_item['tax'],
			  'total_without_vat'=> $cart_item['subtotal'],
